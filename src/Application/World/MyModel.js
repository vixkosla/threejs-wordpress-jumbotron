...
export default class MyModel {
  constructor(scene) {
    ...
    // Чувствительность к ускорению мыши (уменьшено для плавности)
    this.sensitivity = 0.8;
    
    // Максимальная амплитуда "взмаха" (уменьшено)
    this.maxAmplitude = 1.5;
    
    // Длительность возврата (секунды) - "перышко" (увеличено для плавности)
    ...
    this.meshes.forEach((mesh, index) => {
      ...
      if (pushVector && pushVector.length() > 0) {
        ...
        if (velocityDotVector > 0.001) {
          ...
          // Анимация "взмаха" через GSAP (плавно)
